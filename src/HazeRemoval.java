...
public class HazeRemoval {
    ...
    public HazeRemoval(String inFileName) {
        readImage(inFileName);
        inPixels = new int[inImage.getHeight()][inImage.getWidth()];
        guideImage = new double[inImage.getHeight()][inImage.getWidth()];
        tImage = new double[inImage.getHeight()][inImage.getWidth()];
        darkChanel = new double[inImage.getHeight()][inImage.getWidth()];
        outPixels = new int[inImage.getHeight()][inImage.getWidth()];
        setInPixels();
        processDarkChanel();
        setA();
        settImage();
    ...
    public BufferedImage getHazeFreeImage() {
        return outImage;
    }
    ...
    private void setInPixels() {
        ...
        for (int i = 0; i < inImage.getWidth(); i++) {
            for (int j = 0; j < inImage.getHeight(); j++) {
                inPixels[j][i] = inImage.getRGB(i, j);
    ...
    private void processDarkChanel() {
        ...
        MinFilterMatrix(darkChanel, inImage.getHeight(), inImage.getWidth(), 15);
    }

    private void setA() {
        int size = (int) (0.001 * inImage.getHeight() * inImage.getWidth());
        Node[] array = new Node[size];
        int count = 0;
        for (int i = 0; i <= size / inImage.getWidth(); i++) {
            for (int j = 0; j < inImage.getWidth(); j++) {
        ...
        for (int i = size / inImage.getWidth(); i < inImage.getHeight(); i++) {
            if (i == size / inImage.getWidth()) {
                for (int j = size % inImage.getWidth(); j < inImage.getWidth(); j++) {
    ...
    private void settImage() {
        ...
        MinFilterMatrix(tImage, inImage.getHeight(), inImage.getWidth(), 15);
        double max = tImage[0][0];
        double min = tImage[0][0];
        for (int i = 0; i < inImage.getHeight(); i++) {
            for (int j = 0; j < inImage.getWidth(); j++) {
                if (tImage[i][j] > max)
                    max = tImage[i][j];
        ...
        tImage = GuideImageFilter(tImage, inImage.getHeight(), inImage.getWidth(), 105);
        for (int i = 0; i < inImage.getHeight(); i++) {
            for (int j = 0; j < inImage.getWidth(); j++) {
                if (tImage[i][j] > max)
                    max = tImage[i][j];
    ...
    private void HazeFree() {
        ...
        //outPixels = Color_His(outPixels,inImage.getHeight(),inImage.getWidth());
        autoBright(outPixels, inImage.getHeight(), inImage.getWidth());
    }
...
//        double[][] temp = new double[inImage.getHeight()][inImage.getWidth()];
//        for (int i = 0; i < inImage.getHeight(); i++)
//           for (int j = 0; j < inImage.getWidth(); j++)
//                temp[i][j] = tImage[i][j] * 255;
//        temp = BoxFilter(guideImage,inImage.getHeight(),inImage.getWidth(),5);
        for (int i = 0; i < inImage.getHeight(); i++)
            for (int j = 0; j < inImage.getWidth(); j++)
                outImage.setRGB(j, i, outPixels[i][j]);
